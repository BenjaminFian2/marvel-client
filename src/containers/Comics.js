...
const Comics = ({ tokenId, search, modals, setModals }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://benalgo-marvel-server.herokuapp.com/comics?title=${search}&limit=${numItems}&skip=${
            (page - 1) * numItems
          }`
        );
        if (tokenId) {
  ...
  return isLoading ? (
    ...
    <div className="Comics">
      <Pagination
        ...
        setPage={setPage}
      />
      <div className="Comics-containers">
